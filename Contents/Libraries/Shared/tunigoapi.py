...
class Tunigo():
    ...
    def __init__(self, region="us", log_level=1):
        ...
        self.region = region
        self.root_url = "https://api.tunigo.com/v3/space/featured-playlists"
        Logging.debug("Starting with Tunigo. Region: "  + self.region)

    def getFeaturedPlaylists(self):
      fixed_params = "page=0&per_page=20&suppress_response_codes=1&locale=en&product=premium&version=6.25.2&platform=web"
      date_param   = "dt=" + time.strftime("%Y-%m-%dT%H:%M:%S") #2014-05-29T02%3A01%3A00"
      region_param = "region=" + self.region
      full_url = self.root_url + '?' + fixed_params + '&' + date_param + '&' + region_param
